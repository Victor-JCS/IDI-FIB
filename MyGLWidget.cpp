...
{
  radiEsc = sqrt(10*10+10*10+4*4);
  radiEsc1 = sqrt(10*10+10*10+2*2);
  radiEsc2 = sqrt(10*10+10*10);
  radiEsc3 = sqrt(10*10+10*10+2*2);
  radiEsc4 = sqrt(10*10+10*10+4*4);
  centEsc = glm::vec3(0, 4, 0);
}
...
void MyGLWidget::paintGL ()
{
  ...
  switch (patricio){
    case 1 :{
      ...
      modelTransformModel (4*escalaModel, -90.0);
      glDrawArrays(GL_TRIANGLES, 0, m.faces().size()*3);

      glBindVertexArray (VAO_Model);
      modelTransformModel (6*escalaModel, 90.0);
      glDrawArrays(GL_TRIANGLES, 0, m.faces().size()*3);

      glBindVertexArray (VAO_Model);
      modelTransformModel (8*escalaModel, 180.0);
      glDrawArrays(GL_TRIANGLES, 0, m.faces().size()*3);
      break;
    }
    case 2 :{
      glBindVertexArray (VAO_Model);
      modelTransformModel (2*escalaModel, 0.0);
      glDrawArrays(GL_TRIANGLES, 0, m.faces().size()*3);
...
void MyGLWidget::modelTransformModel (float esc, float ang)
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  TG = glm::rotate(TG, glm::radians(ang), glm::vec3(0,1,0));
  TG = glm::translate(TG, glm::vec3(8,0,8));
  TG = glm::rotate(TG, glm::radians(-135.0f), glm::vec3(0,1,0));
...
void MyGLWidget::projectTransform ()
{
  ...
  switch(patricio){
    case 1: {
      Proj = glm::perspective(float(M_PI/2.0), 1.0f, radiEsc1, 3.0f*radiEsc1);
      break;
    }
    case 2: {
      Proj = glm::perspective(float(M_PI/2.0), 1.0f, radiEsc2, 3.0f*radiEsc2);
      break;
    }
    case 3: {
      Proj = glm::perspective(float(M_PI/2.0), 1.0f, radiEsc3, 3.0f*radiEsc3);
      break;
    }
    case 4: {
      Proj = glm::perspective(float(M_PI/2.0), 1.0f, radiEsc4, 3.0f*radiEsc4);
      break;
    }
    default: {
...
void MyGLWidget::viewTransform ()
{
  ...
  switch(patricio){
    case 1: {
      View = lookAt(glm::vec3(8,2,8), centEsc, glm::vec3(0,1,0));
      break;
    }
    case 2: {
      View = lookAt(glm::vec3(-8,4,8), centEsc, glm::vec3(0,1,0));
      break;
    }
    case 3: {
      View = lookAt(glm::vec3(8,6,-8), centEsc, glm::vec3(0,1,0));
      break;
    }
    case 4: {
      View = lookAt(glm::vec3(-8,8,-8), centEsc, glm::vec3(0,1,0));
      break;
    }
    default: {
      ...
      View = glm::translate(View, -centEsc);
    }
  }
